...
#else
    // Set up signal handler for child processes on Unix
...
#else
        // Fork a child process to handle the client on Unix
        pid_t pid = fork();
        if (pid == 0) {
            // Child process
...
DWORD WINAPI handle_client(LPVOID lpParam) {
    ...
    // --- Fix: Rewrite the request line for the origin server ---
    ...
    rest_of_request += 2; // skip "\r\n"

    // Build the new request with only the path in the request line
    ...
    // Forward the rewritten request to the server
...
int parse_http_request(char *request, char *method, char *host, char *path, int *port) {
    ...
    *port = 80;
    ...
    if (strncmp(url, "http://", 7) == 0) {
        ...
        // Make a copy for header parsing
